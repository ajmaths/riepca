...
def _stacked(batched_call, single_call, items, expected_rows):
    ...
    The fallback is never silent: the first time it fires, a ``RuntimeWarning``
    names the exception (or the wrong shape) that triggered it. A batched call
    that fails because of a genuine bug -- a wrong argument order, say -- would
    otherwise become a slow loop that reproduces the same mistake with no
    ...
    try:
        ...
        if array.shape[0] == expected_rows:
            return array
        reason = (
            f"the batched call returned shape {array.shape}, expected a "
            f"leading axis of length {expected_rows}"
        )
    except Exception as exc:  # noqa: BLE001 -- any backend failure falls back
        reason = f"the batched call raised {type(exc).__name__}: {exc}"
    if not _stacked.warned:
        _stacked.warned = True
        warnings.warn(
            "falling back to per-item metric calls because " + reason + ". "
            "Results are unaffected, but this path makes O(n) metric calls "
            "per reference point instead of O(1). This warning is shown once.",
            RuntimeWarning,
            stacklevel=3,
        )
    return np.stack(
...
# is from the truth. The Minakshisundaram--Pleijel expansion on $S^2$ is
#
# $$k_t(x,y)=(4\pi t)^{-1}e^{-d^2/4t}\bigl(u_0(x,y)+t\,u_1(x,y)+O(t^2)\bigr),
# \qquad u_0=\Big(\frac{d}{\sin d}\Big)^{1/2}\approx1+\frac{d^2}{12},
# \qquad u_1(x,x)=\frac{R}{6}=\frac13,$$
#
# so the geodesic Gaussian omits two corrections of the same order: the
# curvature term $t/3$ and the Van Vleck factor $d^2/12$, which for the pairs
# that carry weight ($d^2\sim t$) is comparable to it. The net effect is
# $\approx t/3$ only to leading order at fixed $d$; the relative Frobenius
# difference between the two field arrays, measured on random points and
# references, is about 4.3% at $t=0.1$, 0.37% at $t=0.01$, and 0.21% at
# $t=0.005$.
#
# The exact builder has a second advantage: it never evaluates a logarithm
# map, so it is immune to the cut-locus artefact described in Section 4.
