...
# log-kernel before exponentiating: a mostly underflowed matrix raises a
# warning, and a completely underflowed matrix raises an error with guidance
# to increase t or rescale the data.  No data-dependent rescaling is applied
# silently.
...
def heat_kernel_values(
    ...
    *,
    ...
    underflow_fraction = float(np.mean(log_kernel < _LOG_SMALLEST_SUBNORMAL))
    if underflow_fraction == 1.0:
        raise FloatingPointError(
            "all Euclidean heat-kernel values underflow at this t; increase t "
            "or rescale the coordinates before fitting RiePCA"
        )
    if underflow_fraction >= 0.5 or float(np.median(log_kernel)) < _LOG_SMALLEST_NORMAL:
        warnings.warn(
            f"{underflow_fraction:.1%} of heat-kernel values underflow to zero "
            "and at least half are subnormal or smaller; RiePCA may be "
            "numerically unstable. Increase t or rescale the coordinates.",
            RuntimeWarning,
            stacklevel=2,
        )
...
def _grid_candidates(points, grid_shape, padding):
    ...
    return axes, candidates, grid_shape
...
def _select_grid(
    ...
    *,
    ...
    axes, candidates, grid_shape = _grid_candidates(points, grid_shape, padding)
    ...
    if method == "global_minimum":
        ...
        band, epsilon_used, relative_used, span = _energy_band(
            values, epsilon, relative_epsilon, d, t
        )
...
# ``relative_epsilon=0.30`` is a fraction of the candidate V_t range and is
# invariant to the common heat-kernel normalizer.  It is therefore preferable
...
def _static_reference_result(
    ...
    *,
    ...
    if method == "grid":
        ...
        _, references, resolved_shape = _grid_candidates(
...
def check_block_invariance(result, block, n_trials=5, seed=0, atol=1e-10):
    ...
    if worst_magnitude_change >= atol:
